...
const GSC_CREDENTIALS_PATH = process.env["GSC_CREDENTIALS_PATH"]
    ...
    : null;
const POSSIBLE_CREDENTIAL_PATHS = [
...
/** OAuth 密钥文件是否为用户显式设置 */
const GSC_OAUTH_CLIENT_SECRETS_FILE_EXPLICIT = process.env["GSC_OAUTH_CLIENT_SECRETS_FILE"] !== undefined;
...
/** 跳过 OAuth 认证 */
const SKIP_OAUTH = ["true", "1", "yes"].includes((process.env["GSC_SKIP_OAUTH"] || "").toLowerCase());
/** 安全操作开关 */
const ALLOW_DESTRUCTIVE = ["true", "1", "yes"].includes((process.env["GSC_ALLOW_DESTRUCTIVE"] || "false").toLowerCase());
/** 数据新鲜度 */
const rawDataState = (process.env["GSC_DATA_STATE"] || "all").toLowerCase().trim();
if (!["all", "final"].includes(rawDataState)) {
    throw new Error(`Invalid GSC_DATA_STATE value '${rawDataState}'. ` +
...
async function getOAuthClient() {
    ...
    if (!installed) {
        throw new Error("Invalid OAuth client secrets file. Expected 'installed' or 'web' key with client_id and client_secret.");
    }
    ...
    if (!fs.existsSync(GSC_OAUTH_CLIENT_SECRETS_FILE)) {
        throw new Error("OAuth client secrets file not found. Please place a client_secrets.json file in the script directory " +
...
export async function getGscClient() {
    ...
    if (GSC_CREDENTIALS_PATH && !fs.existsSync(GSC_CREDENTIALS_PATH)) {
        throw new Error(`GSC_CREDENTIALS_PATH is set to ${JSON.stringify(GSC_CREDENTIALS_PATH)} but the file does not exist. ` +
    ...
    if (GSC_OAUTH_CLIENT_SECRETS_FILE_EXPLICIT && !fs.existsSync(GSC_OAUTH_CLIENT_SECRETS_FILE)) {
        throw new Error(`GSC_OAUTH_CLIENT_SECRETS_FILE is set to ${JSON.stringify(GSC_OAUTH_CLIENT_SECRETS_FILE)} ` +
    ...
    throw new Error("Authentication failed. Please either:\n" +
...
export async function reauthenticate() {
    ...
    if (!fs.existsSync(GSC_OAUTH_CLIENT_SECRETS_FILE)) {
        throw new Error("Error: OAuth client secrets file not found. " +
