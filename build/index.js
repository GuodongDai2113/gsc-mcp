 ...
 */
async function gscRequest(client, method, path, body) {
...
/** 站点未找到错误帮助信息 */
function siteNotFoundError(siteUrl) {
    const lines = [`Property '${siteUrl}' not found (404). Possible causes:\n`];
    lines.push("1. The site_url doesn't exactly match what is in GSC. Run list_properties to get the exact string to use.");
    if (siteUrl.startsWith("sc-domain:")) {
        lines.push("2. Domain properties require the service account to be explicitly added under GSC Settings > Users and permissions for that specific domain property. " +
            "OAuth users must also have verified access to it.");
    }
    else {
        lines.push("2. If your property is a domain property (covers all subdomains), the correct format is 'sc-domain:example.com', not a full URL.");
    }
...
server.registerTool("get_capabilities", {
    description: "获取所有可用工具的完整列表、当前认证状态以及入门指南。当被问及有哪些工具可用、此服务器能做什么或如何开始时，始终首先调用此工具。",
}, async () => {
    let authStatus;
    try {
        await getGscClient();
...
server.registerTool("list_properties", {
    description: "列出用户可访问的所有 Google Search Console (GSC) 资产和站点。使用此工具查看已连接 Google Search Console 账户中的所有已验证站点、域名资产和 URL 前缀资产。请始终先调用此工具，以获取其他工具所需的精确 site_url。",
}, async () => {
    try {
        const client = await getGscClient();
        const data = await gscRequest(client, "GET", "webmasters/v3/sites");
        const sites = data.siteEntry || [];
        if (!sites.length)
            return { content: [{ type: "text", text: "No Search Console properties found." }] };
        const result = {
            count: sites.length,
        ...
        return { content: [{ type: "text", text: `Error retrieving properties: ${e instanceof Error ? e.message : String(e)}` }] };
    }
});
server.registerTool("add_site", {
    ...
    try {
        const client = await getGscClient();
        const response = await gscRequest(client, "PUT", `webmasters/v3/sites/${encodeURIComponent(site_url)}`);
        const lines = [`Site ${site_url} has been added to Search Console.`];
        if (response?.permissionLevel)
            lines.push(`Permission level: ${response.permissionLevel}`);
        ...
        if (status === 409)
            return { content: [{ type: "text", text: `Site ${site_url} is already added to Search Console.` }] };
        if (status === 403) {
            // 尝试从错误消息中区分具体原因
            const lowerMsg = msg.toLowerCase();
            if (lowerMsg.includes("forbidden")) {
                return { content: [{ type: "text", text: "Error: You don't have permission to add this site. Please verify ownership first." }] };
            }
            if (lowerMsg.includes("quota") || lowerMsg.includes("quotaexceeded")) {
                return { content: [{ type: "text", text: "Error: API quota exceeded. Please try again later." }] };
            }
            return { content: [{ type: "text", text: `Error: Permission denied. ${msg}` }] };
        }
        if (status === 400) {
            if (msg.toLowerCase().includes("invalid")) {
                return { content: [{ type: "text", text: "Error: Invalid site URL format. Please check the URL format and try again." }] };
            }
            return { content: [{ type: "text", text: `Error: Bad request. ${msg}` }] };
        }
        if (status === 401)
            return { content: [{ type: "text", text: "Error: Unauthorized. Please check your credentials." }] };
        if (status === 429)
            return { content: [{ type: "text", text: "Error: Too many requests. Please try again later." }] };
        if (status === 500)
            return { content: [{ type: "text", text: "Error: Internal server error from Google Search Console API. Please try again later." }] };
        if (status === 503)
            return { content: [{ type: "text", text: "Error: Service unavailable. Google Search Console API is currently down. Please try again later." }] };
        return { content: [{ type: "text", text: `Error adding site (HTTP ${status || "unknown"}): ${msg}` }] };
    }
});
server.registerTool("delete_site", {
    ...
    try {
        ...
        return { content: [{ type: "text", text: `Site ${site_url} has been removed from Search Console.` }] };
    }
    catch (e) {
        const msg = e instanceof Error ? e.message : String(e);
        const status = e.status;
        if (status === 404)
            return { content: [{ type: "text", text: `Site ${site_url} was not found in Search Console.` }] };
        if (status === 403) {
            const lowerMsg = msg.toLowerCase();
            if (lowerMsg.includes("forbidden")) {
                return { content: [{ type: "text", text: "Error: You don't have permission to remove this site." }] };
            }
            if (lowerMsg.includes("quota") || lowerMsg.includes("quotaexceeded")) {
                return { content: [{ type: "text", text: "Error: API quota exceeded. Please try again later." }] };
            }
        ...
        if (status === 503)
            return { content: [{ type: "text", text: "Error: Service unavailable. Google Search Console API is currently down. Please try again later." }] };
        return { content: [{ type: "text", text: `Error removing site (HTTP ${status || "unknown"}): ${msg}` }] };
    }
});
server.registerTool("get_site_details", {
    ...
    try {
        const client = await getGscClient();
        const siteInfo = await gscRequest(client, "GET", `webmasters/v3/sites/${encodeURIComponent(site_url)}`);
        const result = {
            site_url,
            permission_level: siteInfo.permissionLevel || "Unknown",
        };
        if (siteInfo.siteVerificationInfo) {
            const vi = siteInfo.siteVerificationInfo;
            result.verification = {
        ...
        if (siteInfo.ownershipInfo) {
            const oi = siteInfo.ownershipInfo;
            result.ownership = {
        ...
        const msg = e instanceof Error ? e.message : String(e);
        if (e.status === 404) {
            return { content: [{ type: "text", text: siteNotFoundError(site_url) }] };
        }
        return { content: [{ type: "text", text: `Error retrieving site details: ${msg}` }] };
    }
});
server.registerTool("get_search_analytics", {
    ...
    try {
        ...
        const response = await gscRequest(client, "POST", `webmasters/v3/sites/${encodeURIComponent(site_url)}/searchAnalytics/query`, body);
        if (!response.rows?.length) {
            return { content: [{ type: "text", text: `No search analytics data found for ${site_url} in the last ${days} days.` }] };
        }
        ...
        if (msg.includes("404"))
            return { content: [{ type: "text", text: siteNotFoundError(site_url) }] };
        return { content: [{ type: "text", text: `Error retrieving search analytics: ${msg}` }] };
    }
});
server.registerTool("get_performance_overview", {
    ...
    try {
        ...
        const totalResponse = await gscRequest(client, "POST", `webmasters/v3/sites/${encodeURIComponent(site_url)}/searchAnalytics/query`, totalBody);
        // 获取每日趋势
        const dateBody = { ...dateRange, dimensions: ["date"], rowLimit: days, dataState: DATA_STATE };
        const dateResponse = await gscRequest(client, "POST", `webmasters/v3/sites/${encodeURIComponent(site_url)}/searchAnalytics/query`, dateBody);
        if (!totalResponse.rows?.length) {
            return { content: [{ type: "text", text: `No performance data available for ${site_url} in the last ${days} days.` }] };
        }
        ...
        if (msg.includes("404"))
            return { content: [{ type: "text", text: siteNotFoundError(site_url) }] };
        return { content: [{ type: "text", text: `Error retrieving performance overview: ${msg}` }] };
    }
});
server.registerTool("get_advanced_search_analytics", {
    ...
    try {
        ...
        if (!["all", "final"].includes(resolvedDataState)) {
            return { content: [{ type: "text", text: `Invalid data_state value '${params.data_state}'. Accepted values are 'all' or 'final'.` }] };
        }
        ...
        if (params.filters) {
            try {
                ...
                if (!Array.isArray(filterList) || !filterList.length) {
                    return { content: [{ type: "text", text: "Invalid filters value. Expected a non-empty JSON array of filter objects." }] };
                }
                for (const f of filterList) {
                    if (!f.dimension || !f.operator || !f.expression) {
                        return { content: [{ type: "text", text: "Each filter object must have 'dimension', 'operator', and 'expression' keys." }] };
                    }
                }
                ...
                return { content: [{ type: "text", text: "Invalid filters JSON. Please provide a valid JSON array of filter objects." }] };
            }
        }
        ...
        const response = await gscRequest(client, "POST", `webmasters/v3/sites/${encodeURIComponent(params.site_url)}/searchAnalytics/query`, body);
        if (!response.rows?.length) {
            ...
            return { content: [{ type: "text", text: noDataMsg }] };
        }
        const rows = response.rows.map((row) => {
            const entry = {};
            dimList.forEach((dim, i) => {
        ...
        const msg = e instanceof Error ? e.message : String(e);
        if (msg.includes("404"))
            return { content: [{ type: "text", text: siteNotFoundError(params.site_url) }] };
        return { content: [{ type: "text", text: `Error retrieving advanced search analytics: ${msg}` }] };
    }
});
server.registerTool("compare_search_periods", {
    ...
    try {
        ...
        const [p1Response, p2Response] = await Promise.all([
            gscRequest(client, "POST", `webmasters/v3/sites/${encodeURIComponent(site_url)}/searchAnalytics/query`, body1),
            gscRequest(client, "POST", `webmasters/v3/sites/${encodeURIComponent(site_url)}/searchAnalytics/query`, body2),
        ]);
        const p1Rows = p1Response.rows || [];
        const p2Rows = p2Response.rows || [];
        if (!p1Rows.length && !p2Rows.length) {
            return { content: [{ type: "text", text: `No data found for either period for ${site_url}.` }] };
        }
        const p1Data = new Map();
        const p2Data = new Map();
        ...
        comparison.sort((a, b) => Math.abs(b.click_diff) - Math.abs(a.click_diff));
        ...
        if (msg.includes("404"))
            return { content: [{ type: "text", text: siteNotFoundError(site_url) }] };
        return { content: [{ type: "text", text: `Error comparing search periods: ${msg}` }] };
    }
});
server.registerTool("get_search_by_page_query", {
    ...
    try {
        ...
        const response = await gscRequest(client, "POST", `webmasters/v3/sites/${encodeURIComponent(site_url)}/searchAnalytics/query`, body);
        if (!response.rows?.length) {
            return { content: [{ type: "text", text: `No search data found for page ${page_url} in the last ${days} days.` }] };
        }
        const rows = response.rows.map((row) => ({
        ...
        const msg = e instanceof Error ? e.message : String(e);
        return { content: [{ type: "text", text: `Error retrieving page query data: ${msg}` }] };
    }
});
// URL Inspection 工具
server.registerTool("inspect_url_enhanced", {
    ...
    try {
        ...
        const response = await gscRequest(client, "POST", "v1/urlInspection/index:inspect", body);
        if (!response?.inspectionResult) {
            return { content: [{ type: "text", text: `No inspection data found for ${page_url}.` }] };
        }
        ...
        if (inspection.richResultsResult) {
            ...
            richResults = {
                ...
                detected_types: (rich.detectedItems || []).map((i) => i.richResultType || "Unknown"),
                issues: (rich.richResultsIssues || []).map((i) => ({ severity: i.severity, message: i.message })),
            };
        }
        return {
            content: [{
                    ...
                    text: JSON.stringify({
                        ...
                        last_crawled: formatDate(indexStatus.lastCrawlTime),
                        ...
                        referring_urls: (indexStatus.referringUrls || []).slice(0, 5),
...
server.registerTool("batch_url_inspection", {
    ...
    try {
        ...
        if (!urlList.length)
            return { content: [{ type: "text", text: "No URLs provided for inspection." }] };
        if (urlList.length > 10) {
            return { content: [{ type: "text", text: `Too many URLs provided (${urlList.length}). Please limit to 10 URLs per batch.` }] };
        }
        const results = [];
        for (const pageUrl of urlList) {
            try {
                ...
                results.push({
                    ...
                    last_crawled: formatDate(indexStatus.lastCrawlTime) || "Never",
                    rich_results: "None",
                });
            }
            catch (e) {
        ...
        return {
            content: [{ type: "text", text: JSON.stringify({ site_url, count: results.length, results }, null, 2) }],
        };
    }
    catch (e) {
        return { content: [{ type: "text", text: `Error performing batch inspection: ${e instanceof Error ? e.message : String(e)}` }] };
    }
});
server.registerTool("check_indexing_issues", {
    ...
    try {
        ...
        for (const pageUrl of urlList) {
            try {
                ...
                const fetchState = indexStatus.pageFetchState;
        ...
        return { content: [{ type: "text", text: `Error checking indexing issues: ${e instanceof Error ? e.message : String(e)}` }] };
    }
});
// Sitemap 工具
server.registerTool("get_sitemaps", {
    ...
    try {
        const client = await getGscClient();
        const data = await gscRequest(client, "GET", `webmasters/v3/sites/${encodeURIComponent(site_url)}/sitemaps`);
        ...
        const sitemaps = data.sitemap.map((sm) => {
            ...
            return {
                ...
                last_downloaded: formatDate(sm.lastDownloaded),
                status,
        ...
        return {
            content: [{ type: "text", text: JSON.stringify({ site_url, count: sitemaps.length, sitemaps }, null, 2) }],
        };
    }
    catch (e) {
        const msg = e instanceof Error ? e.message : String(e);
        if (msg.includes("404"))
            return { content: [{ type: "text", text: siteNotFoundError(site_url) }] };
        return { content: [{ type: "text", text: `Error retrieving sitemaps: ${msg}` }] };
    }
});
server.registerTool("list_sitemaps_enhanced", {
    ...
    try {
        ...
        const data = await gscRequest(client, "GET", queryPath);
        if (!data.sitemap?.length) {
            return {
                content: [{
                        type: "text",
        ...
        const sitemaps = data.sitemap.map((sm) => {
            ...
            return {
                path: sm.path || "Unknown",
                last_submitted: formatDate(sm.lastSubmitted),
                last_downloaded: formatDate(sm.lastDownloaded),
...
server.registerTool("get_sitemap_details", {
    ...
    try {
        const client = await getGscClient();
        const details = await gscRequest(client, "GET", `webmasters/v3/sites/${encodeURIComponent(site_url)}/sitemaps/${encodeURIComponent(sitemap_url)}`);
        if (!details)
            return { content: [{ type: "text", text: `No details found for sitemap ${sitemap_url}.` }] };
        const isIndex = Boolean(details.isSitemapsIndex);
        const contentBreakdown = (Array.isArray(details.contents) ? details.contents : []).map((c) => ({
            type: (c.type || "unknown").toUpperCase(),
        ...
        return {
            content: [{
                    ...
                    text: JSON.stringify({
                        ...
                        last_submitted: formatDate(details.lastSubmitted),
                        last_downloaded: formatDate(details.lastDownloaded),
        ...
        return { content: [{ type: "text", text: `Error retrieving sitemap details: ${e instanceof Error ? e.message : String(e)}` }] };
    }
});
    ...
    try {
        ...
        try {
            ...
            if (details.lastSubmitted) {
                lines.push(`Submission time: ${formatDate(details.lastSubmitted) || details.lastSubmitted}`);
            }
            ...
            return { content: [{ type: "text", text: `Successfully submitted sitemap: ${sitemap_url}\n\nGoogle will queue it for processing.` }] };
        }
    }
    catch (e) {
        return { content: [{ type: "text", text: `Error submitting sitemap: ${e instanceof Error ? e.message : String(e)}` }] };
    }
});
server.registerTool("delete_sitemap", {
    ...
    try {
        ...
        try {
            ...
            if (e.status === 404) {
                return { content: [{ type: "text", text: `Sitemap not found: ${sitemap_url}. It may have already been deleted or was never submitted.` }] };
            }
            throw e;
        }
        ...
        return { content: [{ type: "text", text: `Error deleting sitemap: ${e instanceof Error ? e.message : String(e)}` }] };
    }
});
server.registerTool("manage_sitemaps", {
    ...
    if (!validActions.includes(action)) {
        return { content: [{ type: "text", text: `Invalid action: ${params.action}. Please use one of: ${validActions.join(", ")}` }] };
    }
    if (["details", "submit", "delete"].includes(action) && !params.sitemap_url) {
        return { content: [{ type: "text", text: `The ${action} action requires a sitemap_url parameter.` }] };
    }
    switch (action) {
        case "list":
            // 直接内联实现 list 操作
            try {
                ...
                const data = await gscRequest(client, "GET", queryPath);
                if (!data.sitemap?.length) {
                    return { content: [{ type: "text", text: `No sitemaps found for ${params.site_url}${params.sitemap_index ? ` in index ${params.sitemap_index}` : "."}` }] };
                }
                const sitemaps = data.sitemap.map((sm) => {
                    let urlCount = null;
                    if (Array.isArray(sm.contents)) {
                ...
                return { content: [{ type: "text", text: JSON.stringify({ site_url: params.site_url, count: sitemaps.length, sitemaps }, null, 2) }] };
            }
            catch (e) {
                return { content: [{ type: "text", text: `Error listing sitemaps: ${e instanceof Error ? e.message : String(e)}` }] };
            }
        case "details":
            // 直接内联实现 details 操作
            try {
                const client = await getGscClient();
                const details = await gscRequest(client, "GET", `webmasters/v3/sites/${encodeURIComponent(params.site_url)}/sitemaps/${encodeURIComponent(params.sitemap_url)}`);
                if (!details)
                    return { content: [{ type: "text", text: `No details found for sitemap ${params.sitemap_url}.` }] };
                const isIndex = Boolean(details.isSitemapsIndex);
                const contentBreakdown = (Array.isArray(details.contents) ? details.contents : []).map((c) => ({
                    type: (c.type || "unknown").toUpperCase(),
                    submitted: c.submitted || 0,
        ...
        case "submit":
            ...
            try {
                ...
                try {
                    ...
                    return { content: [{ type: "text", text: `Successfully submitted sitemap: ${params.sitemap_url}\n\nGoogle will queue it for processing.` }] };
                }
            }
            catch (e) {
                return { content: [{ type: "text", text: `Error submitting sitemap: ${e instanceof Error ? e.message : String(e)}` }] };
        ...
        case "delete":
            if (!ALLOW_DESTRUCTIVE) {
                return { content: [{ type: "text", text: "Safety: delete_sitemap permanently removes a sitemap from GSC. Set GSC_ALLOW_DESTRUCTIVE=true." }] };
            }
            try {
                const client = await getGscClient();
                try {
                    ...
                    if (e.status === 404) {
                        return { content: [{ type: "text", text: `Sitemap not found: ${params.sitemap_url}. It may have already been deleted or was never submitted.` }] };
                    }
                    throw e;
                }
                ...
                return { content: [{ type: "text", text: `Successfully deleted sitemap: ${params.sitemap_url}\n\nNote: This only removes the sitemap from Search Console.` }] };
            }
            catch (e) {
                return { content: [{ type: "text", text: `Error deleting sitemap: ${e instanceof Error ? e.message : String(e)}` }] };
            }
...
server.registerTool("reauthenticate", {
    description: "执行注销和新登录序列。删除当前 OAuth token 文件并触发浏览器认证流程。当需要切换到不同 Google 账户时使用。",
}, async () => {
    try {
        ...
        return { content: [{ type: "text", text: `Error during reauthentication: ${e instanceof Error ? e.message : String(e)}` }] };
    }
});
// =============================================================================
...
async function main() {
    ...
    if (transportEnv === "stdio") {
        ...
        throw new Error(`Unknown MCP_TRANSPORT '${transportEnv}'. Use 'stdio' (default) or 'sse' / 'http'.`);
    }
}
main().catch((error) => {
